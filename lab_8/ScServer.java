...
class ScServer extends Server {
    ...
    ScServer(int numOfScServers, int serverNumber, int maxQSize,
            Supplier<Double> restTimes, double nextAvailableAt) {
        this(numOfScServers, serverNumber, maxQSize, restTimes, 0, nextAvailableAt);
    }

    ScServer(int numOfScServers, int serverNumber,
            int maxQSize, Supplier<Double> restTimes,
    ...
    ScServer(int numOfScServers, int serverNumber, int maxQSize,
            Supplier<Double> restTimes, int totalCustomersServed,
    ...
    ScServer(int numOfScServers, int serverNumber, int maxQSize, Supplier<Double> restTimes,
        ...
        super(serverNumber, maxQSize, restTimes, totalCustomersServed,
                totalCustomerWaitTime, isAvailable,
                nextAvailableAt, customers, false);
        ImList<Server> tempServers = new ImList<Server>();
        for (int i = 1; i < numOfScServers + 1; i++) {
            tempServers = tempServers.add(Server.of(serverNumber + i,
                    1, restTimes, true));
        }
    ...
    Server updateServer(Server server) {
        ImList<Server> newScServers = this.scServers.set(server.getServerNumber()
                - this.getServerNumber() - 1, server);
        return new ScServer(this.scServers.size(), this.getServerNumber(),
                this.getMaxQSize(), this.getRestTimes(),
                this.getTotalCustomersServed(), this.getTotalCustomerWaitTime(), this.isAvailable(),
    ...
    Server popScQueue() {
        ...
        return new ScServer(this.scServers.size(), this.getServerNumber(),
                ...
                this.getTotalCustomerWaitTime(), this.isAvailable(),
